...
const decryptedPrivateKey = decryptSymmetric(
 takenEncryptedPrivateKeyFromDatabase,
 x
);
...
// Later encrypting with privateKey we can later encrypted decrypt with only one place and it wil go descendant

// if i encryptSymmetric

//if i encrypted with public key then i have to decrypt it with private key this could be done with encryptAsymmetric and DecryptAsymmetric
// if i encrypt it userVault then i have to encryptSymmetric and DecryptSymmetric

// 1 step take userVault from user encryptSymmetric encryptPrivateKey,set public and encryptedPrivateKey to database
// 2 step if we want to access some data we decryptSymmetric passing encryptedPrivateKey from userTable and setting his userVault, then if it succeeds
// 3 step in this case when we get privateKey(which was decrypted in the second step) we are encrypting asymmetric(encryptAsymmetric) passing data that we want to encrypt and that user public key, then setting value to database
